...
Run terminal command `./delete_aws_resources.sh` to run script to delete AWS resources: S3 buckets, ClourdWatch Log Groups, EC2 Instances, and RDS Instances.
...
**Note**: the `initial_setup.py` script will create all necessary AWS infrastructure with terraform, upload all model inputs to an AWS S3 bucket, and create and upload a docker image based on the ARTIS codebase in `docker_image_files_original/` directory. It will also submit jobs to the ARTIS HPC.
...
``` default
python3 initial_setup.py -chip arm64 -aws_access_key $AWS_ACCESS_KEY -aws_secret_key $AWS_SECRET_ACCESS_KEY -s3 artis-s3-myname -ecr artis-image-sau-yyyy-mm-dd
```
